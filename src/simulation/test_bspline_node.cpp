...
int main(int argc, char** argv){
	...
	while (ros::ok()){
		...
		// start end condition for bspline
		std::vector<Eigen::Vector3d> startEndCondition; // dummy start end condition (all zeros)
		double currYaw = AutoFlight::rpy_from_quaternion(qm.getOdom().pose.pose.orientation);
		Eigen::Vector3d startCondition (cos(currYaw), sin(currYaw), 0);
		startCondition *= desiredVel;
		...
		ros::Rate rate (10);
		double t = 0.0;
		while (ros::ok() and (not qm.isReach(pGoal, false) or t <= bst.getDuration())){
			qm.setPose(bst.getPose(t));
			// qm.setPose(pwlPlanner.getPose(t));
			t += 0.1;
